...
public class NsnFileModelConverter {
     ...
     *                  output file path.
     */
    public static void ModelConvertWithToc(NeuroshareFile nsObj, String fileFullPath) {
            ...
            for (int i = 0; i < entityCount; i++) {
                ...
                switch (entityType) {
                        ...
                        for (int j = 0; j < ei.getItemCount(); j++) {
                            ...
                            switch ((int) ei.getEntityType()) {
                                case 0:
                                    ...
                                    TextEventData textEventData = (TocUtils.readTextEventDataFromTocFile(tocPath)).get(j);
                                    // ns_EVENT_TEXT
                                    rtnval2 = nsEd.addEventData(textEventData.getTimestamp(), textEventData.getData());
                                    if (rtnval2 != 0) {
                                        // add error. - input arg error - or intermediate file i/o error.
                                    }
                                    break;
                                ...
                                case 2:
                                    // Get Event Data (read TOC)
                                    ByteEventData byteEventData = (TocUtils.readByteEventDataFromTocFile(tocPath)).get(j);
                                    // ns_EVENT_BYTE
                                    rtnval2 = nsEd.addEventData(byteEventData.getTimestamp(), byteEventData.getData());
                                    if (rtnval2 != 0) {
                                        // add error. - input arg error - or intermediate file i/o error.
                                    }
                                    break;
                    ...
                    case 2:
                        ...
                        //  Get Analog Data (read TOC)
                        ArrayList<AnalogData> ad = TocUtils.readAnalogDataFromTocFile(tocPath);
                    ...
                    case 4:
                        ...
                        // Add Neural Event Data (read TOC)
                        ArrayList<Double> d = TocUtils.readNeuralDataFromTocFile(tocPath);
                        for (int jj = 0; jj < d.size(); jj++) {
                            int rtnval9 = nsNED.addNeuralEventData(d.get(jj));
                            if (rtnval9 != 0) {
                                // add error. - input arg error - or intermediate file i/o error.
